...
#FRAME4-1
################################################################################
		self.ToProcessFrame = tkinter.LabelFrame(master=f1, width=300, height=0.75*self.screen_height, pady=5, padx=121)
...
#Normalization Frame	
		...
		self.norm_pre.set('1')
		self.norm_preMenu = tkinter.OptionMenu(self.NormalisationFrame, self.norm_pre, '0','1','2','3','V', command=self.plot_update_trigger2)
		...
		self.norm_postMenu = tkinter.OptionMenu(self.NormalisationFrame, self.norm_post, '0','1','2','3','V', command=self.plot_update_trigger2)
...
################################################################################		
		self.PadFrame = tkinter.LabelFrame(master=f2, width=300, height=0.75*self.screen_height, pady=298, padx=166)
...
################################################################################  
	def plot_spectrum(self):
		...
		def plot_data(evt):
			...
			if '.bin' in self.file_type:
				ang,mu = self.data_read_bin(root_i, self.minr)
...
################################################################################		
	def split(self):
		...
		if '.bin' in self.file_type :
			self.header_read_bin(sef.data_file)
		elif '.qex' in self.file_type:
			self.header_read_qex(self.data_file)
...
################################################################################	
	def batch_export(self):
		...
		if proceed_var == True:
			...
			sub_f = 'batch_extract_subroutine_v2.6.py'
...
#################################################################################		
	def average_trigger(self):
		...
		if self.nsumvar.get() == 'all':
			Nsum = int(np.max(idxs))
		else:
			Nsum = 2*int(self.nsumvar.get())
		
		Nave = int(np.floor((np.max(idxs))/Nsum))
	...
	def save_norm_data(self):  
		if self.NormUsevar.get() == 1:
			...
			if self.AverageUsevar.get() == 1:
				...
				self.norm_export.to_csv(self.folder+'/'+os.path.splitext(self.data_file)[0]+'_normalised_.dat', sep='\t', index=False)
	...
	def plot_spectrum(self):				
		...
		def plot_data(evt):
			...
			if self.initialize_plot == 0:
				...
				autobk(larch_data.energy, larch_data.mu, e0=self.e0_scroll_scale.get(), rbkg=self.rbkg_scroll_scale.get(), group=larch_data, clamp_lo=self.lclamp_scroll_scale.get(), clamp_hi=self.hclamp_scroll_scale.get(), kweight=self.kwback.get(), dk=self.dk_scroll_scale.get(), _larch=mylarch)
				...
				[self.linek] = self.ax2.plot(larch_data.k, larch_data.chi*(larch_data.k**self.kwft.get()), linewidth=2, color='red')
				...
				autobk(larch_data.energy, larch_data.mu, e0=self.e0_scroll_scale.get(), rbkg=self.rbkg_scroll_scale.get(), group=larch_data, clamp_lo=self.lclamp_scroll_scale.get(), clamp_hi=self.hclamp_scroll_scale.get(), kweight=self.kwback.get(), _larch=mylarch)
				...
				self.linek.set_data(larch_data.k, larch_data.chi*(larch_data.k**self.kwft.get()))
		...
		if self.trigger == 0:	
			...
			if self.initialize_ft == 0:
				...
				larch_data.energy = self.data_x
				larch_data.mu = self.data_y
				autobk(larch_data.energy, larch_data.mu, rbkg=self.rbkg_scroll_scale.get(), group=larch_data, clamp_lo=self.lclamp_scroll_scale.get(), clamp_hi=self.hclamp_scroll_scale.get(), kweight=self.kwback.get(), _larch=mylarch)
				xftf(larch_data.k, larch_data.chi, kmin=self.kmin_scroll_scale.get(), kmax=self.kmax_scroll_scale.get(), dk=self.dk_scroll_scale.get(), window='hanning', kweight=self.kwft.get(), group=larch_data, _larch=mylarch)
				xftr(larch_data.r, larch_data.chir, group=larch_data, qmax_out=self.kmax_scroll_scale.get(), rmin=self.rmin_scroll_scale.get(), rmax=self.rmax_scroll_scale.get(), window='hanning', dr=self.dR_scroll_scale.get(), _larch=mylarch)
		...
		if self.trigger == 1:
			...
			if len(self.column_names) == 2:
				...
				for i in range(ds_range):
					self.data_i = i
					self.ds_scroll_scale.set(self.data_i)
					self.update()
