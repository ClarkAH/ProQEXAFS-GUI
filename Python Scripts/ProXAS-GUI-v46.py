...
################################################################################
        ...
        mono = tkinter.Label(self.EncoderFrame, text='Mono Frequency (Hz)',anchor='w')
        mono.grid(column=0, row=2, columnspan=2, rowspan=1, sticky='W')
        
        self.monovar = tkinter.IntVar()
        self.monovar.set(1)
        self.monoentry = tkinter.Entry(self.EncoderFrame, textvariable=self.monovar)
        self.monoentry.grid(column=2, row=2, columnspan=1)    
        ...
        CPUNum = tkinter.Label(self.EncoderFrame, text='CPUs',anchor='w')
    ...
    def spectrumread(self,rooti,minr):
        ...
        if logvar == 1:
            if denomcol == int(len(self.choices)-1):
                mu_r[:,0] = np.log(data_D[numercol::self.nChannels])
            else:
                mu_r[:,0] = np.log(data_D[numercol::int(self.nChannels)]/data_D[denomcol::int(self.nChannels)])
        else:
            if denomcol == int(len(self.choices)-1):
                mu_r[:,0] = data_D[numercol::self.nChannels]
            else:
                mu_r[:,0] = data_D[numercol::self.nChannels]/data_D[denomcol::self.nChannels]
...
################################################################################		
    def split(self):
        ...
        options = [ncpus, np.max(buffer_points), self.headerSize, buffer, encoder_bin, nData, resample_factor, enconder_sampling, mono_freq]       
        sub_f = 'batch_split_subroutine_v46.py'
    ...
    def calibrate(self):
        ...
        if self.initialize == True:
            ...
            def accept_button_on_clicked(mouse_event):
                ...
                calibvalue = [self.cp_scroll_scale.get(), self.thetac, float(self.bf_scroll_scale.get()), self.twod.get()]
    ...
    def process(self):
        ...
        if (self.rootsfile == True) and (self.calibrated == True):
            ...
            try:
                self.calibroots = (pd.read_csv(self.folder+'/roots.dat', sep='\t', header=None)).iloc[:,0].values
                self.calibration_values = (pd.read_csv(self.folder+'/calibration.dat', sep='\t', header=None)).values
                for i in range(len(self.calibroots)-4):
                    ang, mu = self.spectrumread(self.calibroots[i+3],self.calibroots[2])
                    energy = np.transpose(1239.852/(float(self.calibration_values[3])*np.sin((ang+(self.calibration_values[1]-self.calibration_values[0]))*np.pi/180))).tolist()
                    if self.NormUsevar.get() == 1:
                        mu = self.normalize(energy, mu)
                ...
                export.to_csv(export_path+'/'+self.data_bin.split('/')[-1]+'_matrix.dat', sep='\t', index=False)
                self.progress_bar["value"] = 0
...
################################################################################          
        ...
        CPUNum = tkinter.Label(self.InterpFrame, text='CPUs',anchor='w')
...
################################################################################   
    def batch_export(self):
        ...
        sub_f = 'batch_extract_subroutine_v46.py'
        process = subprocess.Popen(['python', sub_f, str(options)], stdout=subprocess.PIPE, shell=True)
