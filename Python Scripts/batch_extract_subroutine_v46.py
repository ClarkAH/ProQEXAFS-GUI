...
def spawn(options):
    ...
    if (rootsfile == True) and (calibrated == True) and (normalized == True):
        for cpu_i in range(n_cpus):
            d = dict(affinity = cpu_i, work = job_indexes[cpu_i], ID = cpu_i, options = options)
...
def wrapper_targetFunc(affinity, work, ID, options):
    proc = psutil.Process()  # get self pid
    proc.cpu_affinity([affinity])
    print('Process-'+str(ID)+' queueing ',work)
    sys.stdout.flush()
    etd = time.time()
    targetFunc(work, etd, ID, options)
...
def targetFunc(work, etd, ID, options):
    ...
    for i in range(work[0], work[1]+1):
        ...
        if skip == False:
            ...
            if ID == 0:
                tps = (time.time()-etd)/(i+1)
                #time_estimate = (work[1]-i)*tps
                print('Percent Complete =',str(100*i*n_cpus/(len(spectra)-n_cpus)))
                print('Time Per Spectrum (s)=',round(tps/(2*n_cpus), 4))
                print('Estimated Time Remaining (s):',int(tps*(work[1]-i-1)/2))
                sys.stdout.flush()
...
def spectrumread(rooti,minr):
    ...
    if logvar == 1:
        if denomcol == int(len(choices)-1):
            mu_r[:,0] = np.log(data_D[numercol::nChannels])
        else:
            mu_r[:,0] = np.log(data_D[numercol::int(nChannels)]/data_D[denomcol::int(nChannels)])
    else:
        if denomcol == int(len(choices)-1):
            mu_r[:,0] = data_D[numercol::nChannels]
        else:
            mu_r[:,0] = data_D[numercol::nChannels]/data_D[denomcol::nChannels]
