...
import tkinter, time, os, psutil, subprocess, sys, shutil, ast, codecs, re, larch, gc, peakutils.peak, itertools, larch
...
class DataExtract(tkinter.Frame):
	def __init__(self,name,*args,**kwargs):
		...
		self.frame3 = tkinter.LabelFrame(self, width=0.7*0.8333*self.screen_height, height=0.3*0.75*self.screen_height)
		self.frame4 = tkinter.LabelFrame(self, width=0.25*0.8333*self.screen_height, height=0.75*self.screen_height)
		self.frame5 = tkinter.LabelFrame(self, width=0.7*0.8333*self.screen_height, height=0.3*0.75*self.screen_height)
		self.frame1.grid(row=0, column=0, rowspan=4, columnspan=1, padx=2)
		...
		self.frame4.grid(row=0, column=3, rowspan=4, columnspan=1, padx=2, pady=15)
...
################################################################################
		def FigureCanvas(self):
			self.fig = plt.figure.Figure(figsize=(6.75, 6.75))
...
################################################################################		
		...
		f1 = tkinter.Frame(master=self.frame4)
...
#FRAME4-1
################################################################################
		self.ToProcessFrame = tkinter.LabelFrame(master=f1, width=300, height=0.75*self.screen_height, pady=21, padx=123)
...
#Encoder Analysis Frame		
		self.EncoderFrame = tkinter.LabelFrame(master=f1, width=300, height=0.75*self.screen_height, pady=5, padx=5)
...
################################################################################
#Calibration Frame	
		self.CalibrationFrame = tkinter.LabelFrame(master=f1, width=300, height=0.75*self.screen_height, pady=5, padx=28)
		...
		self.calib_accept_button=tkinter.Button(self.CalibrationFrame, text='Accept', state='disabled', width=14, command=self.accept_calib).grid(column=1, row=6, stick='E')
		self.calib_cancel_button=tkinter.Button(self.CalibrationFrame, text='Cancel', state='disabled', width=14, command=self.cancel_calib).grid(column=2, row=6, stick='W')
		
################################################################################
#Normalization Frame	
		self.NormalisationFrame = tkinter.LabelFrame(master=f1, width=300, height=0.75*self.screen_height, pady=5, padx=11)
...
################################################################################
#Interpolation Frame
		self.InterpolationFrame = tkinter.LabelFrame(master=f1, width=300, height=0.75*self.screen_height, pady=5, padx=76)
...
################################################################################		
		#self.PadFrame = tkinter.LabelFrame(master=f1, width=310, height=0.75*self.screen_height, pady=25, padx=166)
...
#FRAME4-2
################################################################################
		tkinter.Button(f2, text='Go to DataRead', command=lambda:raise_frame(f1)).grid(row=0, column=0)
...
################################################################################		
		self.PadFrame = tkinter.LabelFrame(master=f2, width=310, height=0.75*self.screen_height, pady=268, padx=166)
...
################################################################################			
		...
		self.bf_scroll_scale = tkinter.Scale(self.scrollFrame, orient='horizontal', from_=0.0005, to=0.995, digits = 4, resolution = 0.0005, state='normal', width=18)
...
################################################################################	   
	def fileplus(self):
		...
		file = askopenfile(filetypes = (("bin files","*.bin"),("qex files","*.qex*"),("all files", "*.*")))
		try:
...
################################################################################      
	def edge_box(self,file):
		...
		label1.grid(column=0, row=2, columnspan=2)
		...
		button1.grid(column=0, row=4, columnspan=1)
		
		button2 = tkinter.Button(self.box, text='Cancel', command=lambda:self.box.destroy())
		button2.grid(column=1, row=4, columnspan=1)
		
		#diffcaliblabel = tkinter.Label(self.box, text='Different Calibration?',anchor='w')
		#diffcaliblabel.grid(column=2, row=4, columnspan=2, rowspan=1, sticky='E') 
		#
		#self.diffcalibUsevar = tkinter.IntVar(self.box)
		#self.diffcalibUsevar.set(0)
		#
		#self.diffcalibUseCheck = tkinter.Checkbutton(self.box, variable=self.diffcalibUsevar, command=lambda e=[self.box.entry3Menu, self.box.entry2], v=self.diffcalibUsevar: self.naccheck(e,v))
		##self.diffcalibUseCheck = tkinter.Checkbutton(self.box, variable=self.diffcalibUsevar)
		#self.diffcalibUseCheck.grid(column=4, row=4)
		#
		#label2 =tkinter.Label(self.box, text='Calibration Element')
		#label2.grid(column=2, row=0, columnspan=2)
		#
		#self.box.entry2 = tkinter.Entry(self.box, state='disabled')
		#self.box.entry2.grid(column=2, row=1, columnspan=2)
		#
		#label3 = tkinter.Label(self.box, text='Enter Edge type')
		#label3.grid(column=2, row=2, columnspan=2)
		#
		#self.box.entry3 = tkinter.StringVar(self.box, state='disabled')
		#self.box.entry3.set('K')
		#self.box.entry3Menu = tkinter.OptionMenu(self.box, self.box.entry3, 'K','L1','L2','L3', state='disabled')
		#self.box.entry3Menu.grid(column=2, row=3, columnspan=2)
 
################################################################################# 
#TO IMPLEMENT
################################################################################# 
	def naccheck_dataprocess(self, entry, var):
		if var.get() == 0:
			for i in entry:
				i.configure(state='disabled')
		else:
			for i in entry:
				if i == self.box.entry3Menu:
					if self.diffcalibUsevar.get() == 1:
						i.configure(state='normal')
					else:
						i.configure(state='disabled')
				elif i == self.box.entry2:
					if self.diffcalibUsevar.get() == 1:
						i.configure(state='normal')
					else:
						i.configure(state='disabled')
...
################################################################################ 
	def data_read_qex(self, root_i, minr):
		...
		#RawData = RawData.groupby('ang', as_index=False).agg({'mu':['mean','sem']})
			...
			if self.trigger == 0:
				...
				if (self.calibrated == True)  & (self.CalibUsevar.get()==1):
					...
					if self.InterpUsevar.get() == 1:
						...
						self.energy, mu = self.interpolate(self.energy, mu, self.xnew)
						if '.bin' in self.file_type:
							self.energy, std = self.interpolate(self.energy, std, self.xnew)
					...
					if self.initialize_plot == 0:
						...
						self.line.set_ydata(mu_raw)
						if (self.Filtervar.get() == 1) or (self.InterpUsevar.get() == 1):
							self.filter_line.set_ydata(mu)
						self.canvas.draw_idle()
...
################################################################################		
	def split(self):
		...
		sub_f = 'batch_split_subroutine_v2.1.py'
...
################################################################################  
	def delete_split(self):
		...
		print('Suggest increasing refactor value')
...
################################################################################		
	def calibrate(self):
		...
		self.calib_accept_button=tkinter.Button(self.CalibrationFrame, text='Accept', state='normal', width=14, command=self.accept_calib).grid(column=1, row=6, stick='E')
		self.calib_cancel_button=tkinter.Button(self.CalibrationFrame, text='Cancel', state='normal', width=14, command=self.cancel_calib).grid(column=2, row=6, stick='W')
			
		self.cp_scroll_scale = tkinter.Scale(self.scrollFrame, orient='horizontal', from_=float(self.min_ang), to=float(self.max_ang), digits = 6, resolution = 0.00005, state='normal', width=18, command=self.plot_cp_line)
		self.cp_scroll_scale.grid(column=1, columnspan=1, row=2, stick='EW')
...
################################################################################                 
	def regression(self,region,x,y): 
		...
		def quartic(x, a, b, c, d, e):
			return (a*x) + (b*x**2) + (c*x**3) + (d*x**3) + e
...
class PostProcess(tkinter.Frame):
	def __init__(self,name,*args,**kwargs):
		...
		self.frame3 = tkinter.LabelFrame(self, width=0.25*0.8333*self.screen_height, height=0.3*0.75*self.screen_height)
		self.frame4 = tkinter.LabelFrame(self, width=0.25*0.8333*self.screen_height, height=0.75*self.screen_height)
...
################################################################################
#Average Frame		
		self.AverageFrame = tkinter.LabelFrame(master=f1, width=300, height=0.75*self.screen_height, pady=5, padx=65)
		...
		self.buttonaverage.grid(column=0, row=3, columnspan=3, rowspan=1)	
		
		self.saveaverage= tkinter.Button(self.AverageFrame, text='Save Average', width=30, height=1, command=self.saveaverage_trigger)
		self.saveaverage.grid(column=0, row=4, columnspan=3, rowspan=1)	
...
################################################################################
#Time Filter Frame		
		self.TimeFrame = tkinter.LabelFrame(master=f1, width=300, height=0.75*self.screen_height, pady=5, padx=65)
...
################################################################################
		...
		self.PadFrame = tkinter.LabelFrame(master=f2, width=300, height=0.75*self.screen_height, pady=439, padx=166)
		self.PadFrame.grid(row=4, column=0)
...
#################################################################################		
	def average_trigger(self):
		...
		if self.nsumvar.get() == 'all':
			Nsum = int(np.max(idxs)+2)
		else:
			Nsum = 2*int(self.nsumvar.get())
			
		Nave = int(np.floor((np.max(idxs)+2)/Nsum))
...
#################################################################################	
	def saveaverage_trigger(self):
		try:
			self.data_sum
			self.data_sum.to_csv(self.folder+'/'+os.path.splitext(self.data_file)[0]+'_average_'+str(self.nsumvar.get())+'.dat', sep='\t', index=False)
			if self.errors_flag == True:
				self.data_sum_error.to_csv(self.folder+'/'+os.path.splitext(self.data_file)[0]+'_average_'+str(self.nsumvar.get())+'.error', sep='\t', index=False)
			
			print('Average Data Saved')
		except:
			print('Averaged Data Not Found')
...
################################################################################ 
	def save_norm_data(self):  
		if self.NormUsevar.get() == 1:
			...
			#self.norm_export_errors = pd.DataFrame()
...
class FourierTransform(tkinter.Frame):
	def __init__(self,name,*args,**kwargs):
		...
		self.frame2 = tkinter.LabelFrame(self, width=0.5*0.8333*self.screen_height, height=0.7*0.75*self.screen_height)
		self.frame3 = tkinter.LabelFrame(self, width=0.25*0.8333*self.screen_height, height=0.3*0.75*self.screen_height)
		self.frame4 = tkinter.LabelFrame(self, width=0.25*0.8333*self.screen_height, height=0.3*0.75*self.screen_height)
		self.frame1.grid(row=0, column=0, rowspan=4, columnspan=1, padx=2)
...
################################################################################
		...
		self.buttonbatch= tkinter.Button(self.BFTFrame, text='Batch Export', width=30, height=1, command=self.batch_trigger)
...
################################################################################				
	def update_figure(self, ax, labelx, labely):
		self.clear_figure()
		
		#if ax == 1:
		#	self.ax1.set_xlabel(labelx)
		#	self.ax1.set_ylabel(labely)
		#if ax == 2:
		#	self.ax2.set_xlabel(labelx)
		#	self.ax2.set_ylabel(labely)
		#if ax == 3:
		#	self.ax3.set_xlabel(labelx)
		#	self.ax3.set_ylabel(labely)
	...
	def plot_spectrum(self):				
		...
		def plot_data(evt):
			...
			if self.initialize_plot == 0:
				...
				self.update_figure(1, 'Energy (eV)', 'Normalised Absorption')
		...
		if self.trigger == 1:
			...
			self.rmcr.to_csv(self.folder+'/FT/'+str(os.path.splitext(self.data_file)[0])+'/R-space_ri_mcr.dat', sep='\t', index=False)
			self.dfrm.to_csv(self.folder+'/FT/'+str(os.path.splitext(self.data_file)[0])+'/R-space_magnitude.dat', sep='\t', index=False)
			self.dfk.to_csv(self.folder+'/FT/'+str(os.path.splitext(self.data_file)[0])+'/K-space.dat', sep='\t', index=False)
			self.dfq.to_csv(self.folder+'/FT/'+str(os.path.splitext(self.data_file)[0])+'/Q-space.dat', sep='\t', index=False)
			...
			self.chiE.to_csv(self.folder+'/FT/'+str(os.path.splitext(self.data_file)[0])+'/chiE.dat', sep='\t', index=False)	
			self.chiKw.to_csv(self.folder+'/FT/'+str(os.path.splitext(self.data_file)[0])+'/chikw.dat', sep='\t', index=False)	
			if self.errors_flag == True:
				self.chiKwerror.to_csv(self.folder+'/FT/'+str(os.path.splitext(self.data_file)[0])+'/chikw.error', sep='\t', index=False)	
