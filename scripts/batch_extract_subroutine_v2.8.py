...
def data_read_qex(root_i, minr):
	...
	RawData = RawData.groupby('ang', as_index=False).agg({'mu_sam':['mean','sem'], 'mu_ref':['mean','sem'], 'time':'mean'})
	RawData.columns = ['ang', 'mu_sam', 'std_sam', 'mu_ref', 'std_ref', 'time']
...
def data_read_bin(root_i, minr):
	...
	return np.around(RawData['ang'].values, decimals=5), RawData['mu_sam'].values, RawData['mu_ref'].values, RawData['sem_sam'].values, RawData['sem_ref'].values, flag
