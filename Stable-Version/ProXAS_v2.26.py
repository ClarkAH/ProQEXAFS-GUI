			...
			if self.trigger == 0:
				...
				if (self.calibrated == True)  & (self.CalibUsevar.get()==1):
					...
					if self.InterpUsevar.get() == 1:
						...
						self.energy, mu = interpolate.localised(self.energy, mu, self.xnew)
						if '.bin' in self.file_type:
							self.energy, std = interpolate.localised(self.energy, std, self.xnew)
...
################################################################################	
	def batch_export(self):
		...
		if proceed_var == True:
			...
			sub_f = 'batch_extract_subroutine_v2.9.py'
...
#Average Frame		
		...
		self.buttonaverage.grid(column=0, row=5, columnspan=3, rowspan=1)	
		
		self.saveaverage= tkinter.Button(self.AverageFrame, text='Save Average', width=30, height=1, command=self.saveaverage_trigger)
		self.saveaverage.grid(column=0, row=6, columnspan=3, rowspan=1)	
...
################################################################################		
		...
		self.buttonsvd= tkinter.Button(self.AnalzeFrame, text='SVD', width=30, height=1,command=self.svd)
		...
		self.buttonsimplisma= tkinter.Button(self.AnalzeFrame, text='SIMPLISMA', width=30, height=1,command=self.simplisma)
		self.buttonsimplisma.grid(column=0, row=3, columnspan=2, rowspan=1)
		
		self.buttonsimplismasave= tkinter.Button(self.AnalzeFrame, text='Save', width=30, height=1,command=self.simplisma_save)
		...
		self.buttonloadcomponents= tkinter.Button(self.AnalzeFrame, text='Load Components', width=30, height=1,command=self.loadcomponents)
		self.buttonloadcomponents.grid(column=0, row=5, columnspan=2, rowspan=1)
		
		self.buttonlcf= tkinter.Button(self.AnalzeFrame, text='LCF', width=30, height=1,command=self.lcf)
		self.buttonlcf.grid(column=2, row=5, columnspan=2, rowspan=1)
		
		self.buttonmcr= tkinter.Button(self.AnalzeFrame, text='MCR', width=30, height=1,command=self.mcr)
		self.buttonmcr.grid(column=0, row=10, columnspan=3, rowspan=1)
...
################################################################################	
	def simplisma(self):
		...
		self.S, self.C_u, self.C_c, self.LOF = simplisma.pure(self.D.T, self.simplisma_number.get(), 5, True)
...
################################################################################	
	def lcf(self):
		...
		self.C_c, self.LOF = lcf.lcf(self.D.T, self.S)
...
################################################################################	
	def mcr(self):
		...
		mcrals.fit(self.D, ST=self.S.T, verbose=True)
		print('\nFinal MSE: {:.7e}'.format(mcrals.err[-1]))
		
		self.CS = np.dot(mcrals.C_opt_, mcrals.ST_opt_).T
		
		LOF = np.zeros(np.shape(self.D.T)[1])
		for i in range(np.shape(self.D.T)[1]):
			LOF[i] = 100*np.sqrt(np.sum(self.D.T[:,i]-self.CS[:,i])**2/np.sum(self.D.T[:,i])**2)

		self.clear_figure()
		self.ax1=self.fig.add_subplot(3, 1, 1)
		self.ax1.plot(mcrals.ST_opt_.T)

		self.ax2=self.fig.add_subplot(3, 1, 2)
		self.ax2.plot(mcrals.C_opt_)
